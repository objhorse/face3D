...
def _readonly_array_mapping(
    ...
    return MappingProxyType(
        {str(name): _readonly_array(value, dtype=dtype) for name, value in values.items()}
    )
...
class NasalSemanticFrame:
    ...
    def __post_init__(self) -> None:
        ...
        rotation = _readonly_array(self.model_to_front_camera, dtype=np.float64)
...
class NasalSemanticBasis:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "region_masks", _readonly_array_mapping(self.region_masks, bool)
        )
        object.__setattr__(
            self, "seed_indices", _readonly_array_mapping(self.seed_indices, np.int64)
        )
    ...
    def validate(self, vertex_count: Optional[int] = None) -> None:
        ...
        if set(self.seed_indices) != set(_NASAL_SEED_NAMES):
            raise ValueError("basis seed metadata is incomplete")
...
def _validate_rotation(rotation: np.ndarray) -> np.ndarray:
    ...
    if not np.allclose(value @ value.T, np.eye(3), atol=1e-7, rtol=0.0):
        raise ValueError("model-to-front-camera rotation must be orthonormal")
...
def build_nasal_semantic_basis(
    ...
    lmk_tri_vidx: np.ndarray,
    lmk_bary_coords: np.ndarray,
    model_to_front_camera: Optional[np.ndarray] = None,
    ...
    rotation = _validate_rotation(
        ...
        if model_to_front_camera is None
        else model_to_front_camera
    )
    ...
    for raw_mode in raw_modes:
        raw = np.asarray(raw_mode, dtype=np.float64).copy()
        raw[protected_mask | ~global_support] = 0.0
    ...
    support_mask = np.any(mode_support_np, axis=0)
    region_masks = {
    ...
    metadata = {
        ...
        "normalization": (
            "paired weighted-L2 balance and per-mode fixed peak amplitude"
        ),
