...
默认三档锚定权重以当前 `LAMBDA_SHAPE = 5e-4` 为尺度，固定为 `5e-4 / 2e-3 / 8e-3`（即 `1x / 4x / 16x`）。极弱平均脸先验默认为 `5e-6`，是第一档身份锚定的 `1%`。这些值是 pipeline 级默认配置，不能为单个人脸动态改写；实验只能通过显式配置覆盖，并必须写入报告。
...
每个 checkpoint 除现有可信轮廓、内部五官和网格门禁外，再执行 identity drift 门禁。候选选择只能发生在全部门禁通过的 checkpoint 中。
